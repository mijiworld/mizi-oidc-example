...
[빠른 시작](#quickstart) · [로그인 유지 정책](#로그인-유지-정책) · [세션과 데이터](#세션과-데이터) · [내 소개 쓰기 체험](#내-소개-쓰기-체험)
...
8. **내 정보 → 내 소개 수정 허용하기**에서 별도 동의를 받은 뒤 소개를 입력하고 **미지에 저장**합니다. 실제 소개가 바뀌며 재조회 결과까지 일치해야 반영 확인을 표시합니다.
9. **이 데모에서 로그아웃**한 뒤 같은 미지 계정으로 다시 로그인해 같은 회원 ID인지 확인합니다. 최초 동의에서 거절하는 경우 데모의 로그인 세션이 만들어지지 않아야 합니다. 이미 동의한 앱은 다음 연결 때 동의 화면을 생략할 수 있습니다.
...
이 예제는 **public client + PKCE**이므로 client secret이 필요 없습니다. 첫 로그인은 `openid profile`만 요청합니다. 내 정보 가져오기는 `user:profile`, 스킬 가져오기는 `user:skills`도 추가로 요청합니다. 등록 후 각 기능에서 필요한 권한을 사용자에게 동의받습니다. 등록 자체가 사용자 승인은 아닙니다. `/v1/me/profile` 응답에는 연락처·지역도 포함되지만 데모 서버는 이를 즉시 제외하고 저장하거나 표시하지 않습니다. 토큰 교환과 API 조회는 서버에서 처리하므로 브라우저 CORS 허용 출처를 추가할 필요가 없습니다.
...
`/profile`의 **내 소개 수정 허용하기 → 미지에서 추가 동의 → 소개 입력 → 미지에 저장**으로 실제 미지 프로필의 소개만 수정합니다. 기존 공개 설정에 따라 공개 프로필에도 반영됩니다. 첫 로그인·일반 읽기 연결에서는 쓰기 권한을 요청하지 않습니다.

- 명시적 `POST /connect-profile-write`만 `user:profile:write`와 고정 `/v1/me/profile/bio` resource를 추가 요청합니다. 읽기 결과 확인에는 `user:profile`도 필요합니다.
- 브라우저는 `POST /profile/bio`로 소개만 보냅니다. 서버는 저장한 접근 토큰(필요 시 먼저 갱신)을 사용해 `PATCH /v1/me/profile/bio`에 `{ "bio": "소개" }`를 한 번 보냅니다.
- 최대 300 UTF-16 코드 단위, 자동 trim 없음, 빈 문자열은 삭제입니다. 다른 필드를 함께 보낼 수 없습니다. 응답은 `{ "user": { "id": "usr_example" }, "bio": "소개" }`입니다.
- PATCH 응답과 후속 `GET /v1/me/profile`의 회원 ID·소개가 모두 일치해야 저장과 반영 확인을 표시합니다. 오류나 통신 끊김에는 자동 재시도하지 않고 저장 거절·저장 후 재조회 실패·저장 여부 불확실을 구분합니다.
- 안전한 결과와 입력 초안은 서버 세션에 저장합니다. 프로필 재조회 성공 시 이전 알림·초안을 지우며, 로그아웃/세션 만료 후 결과 저장으로 세션이 복구되지 않습니다. URL·로그·브라우저에 토큰은 보내지 않습니다.

자세한 계약과 처리 순서는 공개 `/developers#dg-write`, `src/profile-write.ts`, `src/app.ts`, `src/profile-write-notice.ts`를 참고하세요. 기존 전체 `PATCH /v1/me/profile`은 위임 토큰으로 사용할 수 없습니다.
