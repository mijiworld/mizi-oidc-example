...
7. 연결된 상태에서 **프로필 다시 가져오기** 또는 **스킬 다시 가져오기**를 누릅니다. 미지로 이동하지 않고 해당 화면만 새로 조회하며 프로젝트 선택과 세션 만료 시각은 유지됩니다. 연결이 없거나 만료됐다면 **다시 연결하기**로 한 번 연결하세요. 자동으로 미지 화면을 열지는 않습니다.
...
- **연결 후 재조회:** `POST /refresh-profile`은 `/v1/me`와 `/v1/me/profile`만, `POST /refresh-skills`는 `/v1/me/skills`만 새로 읽습니다. OAuth를 시작하지 않고 다른 화면의 결과·프로젝트 선택·세션 ID·만료 시각을 유지합니다. 최초 연결이나 만료 후 재연결은 별도 `POST /connect-profile`, `POST /connect-skills`로 사용자가 직접 시작합니다.
...
API 응답은 **조회 당시의 요약**입니다. 다시 가져오기는 서버 전용으로 보관한 접근 토큰을 사용합니다. 통신 실패·잘못된 응답에는 이전 결과와 조회 시각을 유지하고, 프로필 일부만 성공하면 성공한 결과만 갱신합니다. 만료·401/403 등으로 연결을 사용할 수 없으면 토큰을 제거하고 명시적 재연결을 안내합니다. 보드에는 기본 회원 API(`/v1/me`) 성공이 필요하며, 소개나 스킬 조회 실패가 기본 회원 API의 성공을 지우지는 않습니다. 회원 ID 불일치는 인증 경계 오류로 처리하고 다른 회원 정보를 저장하지 않습니다.
...
스킬 조회는 콜백과 재조회에서 API의 다음 커서를 따라가며 최대 **200개·세션에 저장할 요약 192 KiB·10페이지·5초** 안에서 목록을 모읍니다. 응답 본문은 페이지당 256 KiB, 누적 1 MiB로 제한하며, 최초 연결에서는 콜백 처리 시작 후 12초까지의 공통 API 시간 제한도 적용합니다. 재조회는 토큰 만료 시각과 5초 중 이른 제한을 적용합니다. 먼저 도달한 제한에서 멈추고, 첫 성공 페이지 뒤에 오류가 나면 이미 읽은 목록과 중단 이유를 남깁니다. 단, 재조회 중 어느 페이지든 401/403이 발생하면 이번 목록을 저장하지 않고 이전 목록·조회 시각을 유지하며 접근 토큰을 제거하고 재연결을 안내합니다. 같은 ID는 처음 받은 값을 유지합니다. `collection`에는 조회 페이지 수·시작 시각·중단 이유·중복 수를 보관하며 커서는 남기지 않습니다. 스킬 요약에는 토큰을 넣지 않습니다. `truncated`는 수집 제한이나 중단을, `hasMore`는 마지막 성공한 API 응답의 다음 커서 유무를 뜻합니다. 화면에서 아직 펼치지 않은 개수와는 별개입니다.

**더 보기**는 `/skills?shown=40#skill-21` 같은 데모 내부 GET으로 저장된 목록만 펼칩니다. 추가 API 호출·동의·세션 교체가 없고 프로젝트 선택이나 만료 시각도 바꾸지 않습니다. 예전 세션에 재조회 연결이 없다면 한 번 **다시 연결하기**를 사용해야 새 목록을 읽을 수 있습니다. 이후 유효한 연결에서는 **다시 가져오기**로 해당 API만 조회합니다.

목표는 현재 세션의 남은 기간에만 저장되며, 선택 변경·API 재조회로 30분 만료를 연장하지 않습니다. 명시적 재연결로 같은 issuer·회원임을 확인하고 새 기본 회원 조회까지 성공하면 유효한 기존 세션의 목표를 새 세션에 이어갑니다. 다른 계정에는 이전 회원 정보나 목표를 넘기지 않습니다. 로그아웃하면 목표도 삭제됩니다. 미지에서 연결을 해제해도 이미 저장된 요약과 데모 세션이 즉시 사라지는 것은 아니므로, 즉시 지우려면 데모에서도 로그아웃하세요.
...
- 추가 API 동의로 받은 **접근 토큰만 서버 전용 필드**에 보관합니다. 운영 DynamoDB는 저장 시 암호화(SSE)를 사용합니다. 일반 세션 조회와 화면 모델에서는 이 필드를 제거하며 브라우저·로그·GitHub에 원문을 전달하지 않습니다. ID·갱신 토큰은 보관하지 않습니다. 화면에 사용하는 세션에는 회원 ID·닉네임과 검증 결과, API의 최소 조회 결과와 프로젝트 목표만 포함됩니다.
- 현재 미지 접근 토큰 응답의 `expires_in`은 3600초이지만, 이 데모는 실제 토큰 만료와 30분 세션 만료 중 이른 시각까지만 사용합니다. 재조회는 이를 연장하지 않습니다. 접근 토큰이 없거나 만료되면 사용자가 명시적으로 다시 연결해야 하며 갱신 토큰을 이용한 자동 갱신은 구현하지 않습니다.
- 로그아웃·계정 교체 시 이전 서버 세션과 접근 토큰을 삭제합니다. DynamoDB TTL의 물리 삭제는 지연될 수 있지만, 앱은 만료 시각을 매 요청에서 검사하므로 만료된 세션·토큰을 사용할 수 없습니다.
