...
4. 프로젝트 보드까지 체험하려면 **내 미지 정보 가져오기**를 누르고 `user:profile` 추가 권한에 동의합니다. 이 단계는 선택 사항이며 건너뛰어도 로그인은 완료된 상태입니다. 데모 서버가 `GET /v1/me`를 호출하고 닉네임·GitHub 연결 상태와 조회 시각을 표시합니다. API의 회원 ID가 로그인한 회원과 같은지도 검사합니다.
