...
function skillsPanel(model: HomeViewModel): string {
  ...
  return `<section class="card stage" aria-labelledby="skills-heading"><div class="stage-heading"><h2 id="skills-heading">${skills ? "조회한 스킬 목록" : "내 스킬 가져오기"}</h2>${skills ? "" : '<span class="tag">선택</span>'}</div>
    ...
    <details class="developer-note"><summary>조회 범위와 검증 정보 읽는 법</summary><p>처음 연결할 때는 <code>user:profile</code>과 <code>user:skills</code>에 동의해 내 정보도 함께 가져옵니다. 연결 후 다시 가져오기는 <code>GET /v1/me/skills?limit=20</code>만 호출하고 프로필 결과는 유지합니다. 다음 커서가 있으면 이어서 읽되 최대 200개·10페이지·5초, 보관할 스킬 정보 192KiB 한도에서 멈춥니다.</p><p>접근·갱신 토큰은 서버 전용 저장소에서만 사용하며 ID 토큰은 보관하지 않습니다. 다시 가져오기 때 필요한 토큰 갱신은 서버가 처리합니다. ‘더 보기’는 저장된 목록을 20개씩 펼치며 API를 다시 호출하지 않습니다. 정상 방문은 로그인 미사용 기한을 갱신하지만 90일 절대 한도와 프로젝트 선택은 유지합니다.</p><p>검증 방법·주체·시각은 API가 제공한 정보이며, 이 데모의 별도 검증이나 추천을 뜻하지 않습니다. 검증 시각이 없으면 조회 시각으로 대신하지 않습니다. 마지막 API 응답의 페이지 안내만으로 전체 스킬을 모두 가져왔다고 판단하지 않습니다.</p>${skills ? `<dl class="data">${row("조회 대상 · 로그인 회원", skills.subject)}${row("API", skills.endpoint)}${row("읽은 API 응답 항목 수 · 중복 포함", String(skills.returnedCount))}${row("마지막 응답의 후속 페이지 안내", skills.hasMore === null ? "정보 없음" : skills.hasMore ? "있음" : "없음")}${skills.collection ? `${row("수집 시작 시각 · UTC", skills.collection.startedAt)}${row("읽은 API 페이지 수", String(skills.collection.pages))}${row("중복으로 제외한 항목 수", String(skills.collection.duplicateCount))}${row("수집 종료 이유", skills.collection.stoppedReason)}` : ""}</dl><p>조회 대상은 검증된 로그인 계정입니다. 개별 스킬의 소유자 정보는 이 API 응답에 포함되지 않습니다.</p>` : ""}</details>
  </section>`;
}
...
function projectPanel(model: HomeViewModel, ready: boolean): string {
  ...
  return `<section id="project-board" class="card stage" aria-labelledby="project-heading">
    ...
    ${
      ...
      <p class="fine">선택은 이 데모의 유효한 로그인 세션에 보관되며, 세션 만료나 로그아웃 시 사라집니다.</p>`
        : ""
    }
...
function success(
  ...
  const lead =
    page === "profile"
      ...
      : page === "skills"
        ...
        : "서비스 종류를 고르고 시작 목록을 체험해 보세요. 선택은 이 데모의 로그인 기간 동안 보관돼요.";
  const body =
    page === "home"
  ...
  return `${model.error ? `<section class="error" role="alert"><h2>이번 연결을 완료하지 못했어요</h2><p>${escape(model.error)}</p><p>기존 로그인은 유지됩니다.${member ? " 회원 정보는 이전 조회 결과입니다." : " 해당 정보 화면에서 다시 시도해 주세요."}</p></section>` : ""}
    ...
    <div class="session-actions"><form action="${escape(model.logoutAction)}" method="post"><button class="button secondary" type="submit">이 데모에서 로그아웃</button></form><p class="fine">새 로그인부터 30일 미사용 또는 실제 미지 인증 후 최대 90일에 다시 로그인합니다. 이 데모의 로그아웃은 미지·Google의 로그인까지 종료하지 않습니다.</p></div>
    ${resources()}`;
}
